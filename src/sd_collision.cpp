...
static f32 calculateDesiredDeltaVelocity(SDContact *contact, f32 duration) {
    ...
    if(contact->body[0]->is_awake) {
        velocity_from_acc += sd_vec3_dot(contact->body[0]->last_frame_acceleration * duration, contact->contact_normal);
    }
    if(contact->body[1] && contact->body[1]->is_awake) {
        velocity_from_acc -= sd_vec3_dot(contact->body[1]->last_frame_acceleration * duration, contact->contact_normal);
    }
    // is the velocity is very slow limit the restitution
    f32 this_restitution = 0.6f; // TODO: ????
...
static void apply_position_change(SDContact *contact, SDVec3 linear_change[2], SDVec3 angular_change[2], f32 penetration) {
    ...
    for(u32 i = 0; i < 2; i++) {
        if(contact->body[i]) {
            SDMat3 inverse_inertia_tensor = contact->body[i]->inverse_inertia_tensor;
...
static void resolve_interpenetration(SDCollisionResolver *cr, SDContact *c, u32 contacts_count, f32 duration) {
    u32 i, index;
    SDVec3 linear_change[2], angular_change[2];
    f32 max;
...
static void apply_velocity_change(SDContact *contact, SDVec3 velocity_change[2], SDVec3 rotation_change[2]) {
    ...
    inverse_inertia_tensor[0] = contact->body[0]->inverse_inertia_tensor;
    if(contact->body[1]) {
        inverse_inertia_tensor[1] = contact->body[1]->inverse_inertia_tensor;
    }

    // we will calculate the impulse for each contact axis
    SDVec3 impulse_contact = calculate_frictionless_impulse(contact, inverse_inertia_tensor);
    ...
    if(contact->body[1]) {
        ...
        contact->body[1]->velocity += velocity_change[0];
        contact->body[1]->rotation += rotation_change[0];
    }
}

static void resolve_velocities(SDCollisionResolver *cr, SDContact *c, u32 contacts_count, f32 duration) {
    SDVec3 velocity_change[2], rotation_change[2];
    SDVec3 delta_vel;
