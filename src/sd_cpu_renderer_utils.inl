...
static void DrawTriangleAnim(SDAnimator *animator, SDVertex *vertices, SDMat4& view_world, SDMat4& proj, SDTexture *texture, float *depthBuffer) {
    Vec4 transformVertex[3];
    for(int i = 0; i < 3; i++)
    {
        ...
        for(i32 j = 0; j < 4; j++) {
            ...
            Vec4 local_position = animator->final_bone_matrices[vertices[i].bone_id[j]] * Vec4(vertices[i].pos.x, vertices[i].pos.y, vertices[i].pos.z, 1.0f);
